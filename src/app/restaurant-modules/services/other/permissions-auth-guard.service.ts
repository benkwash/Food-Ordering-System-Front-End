...
@Injectable()
export class PermissionsAuthGuardService  implements CanActivate,CanActivateChild{
  	...
  	canActivate(route: ActivatedRouteSnapshot,state:RouterStateSnapshot):Observable<boolean> | Promise<boolean> | boolean
	{
		...
		const permission=route.data.yoyoPermission;
        ...
        if(this.authService.isAuthenticated()){            
            ...
            if(this.authService.isAdmin()) {
                return true;
            }else if(this.authService.isStaff()){ //if useris a staff(user type ==staff)
				//check whether the staff has permission to access this route
