...
export class SignUpComponent implements OnInit {
	...
	// emailcont = new FormControl('', [Validators.required, Validators.email]);
	// emailtest=""

	// getErrorMessage() {
	// 	if (this.emailcont.hasError('required')) {
	// 	  return 'You must enter a value';
	// 	}
	
	// 	return this.emailcont.hasError('email') ? 'Not a valid email' : '';
	//   }
	...
	public onEmailChange():void
	{
		...
		// this.form.controls.email.setValue({status:'INVALID'})
	...
	public onSubmitSignUp()
	{
			...
			&& !this.isSubmitting)
		{
			...
			let formToSubmit = {
				name: this.formState.fName.value.trim(),
			...
			this.appHttpOp.submitSignUp(formToSubmit)
				.subscribe((response:any) =>
				{
	...
	public resendVerificationEmail():void
	{
		if(!this.isSubmitting)
		{
			...
			this.appHttpOp.resendAdminAcctVerification(this.verificationID)
				.subscribe((response:any)=>
				{
					this.isSubmitting = false;
	...
	public onSubmitVerificationCode()
	{
		...
		if(this.verificationCode && !this.isSubmitting)
		{
			...
			this.appHttpOp.submitAcctVerification(this.verificationID,this.verificationCode)
				.subscribe((response:any)=>
				{
					this.isSubmitting = false;
