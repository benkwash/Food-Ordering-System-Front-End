...
@Injectable()
export class CustomerAuthGuardService  implements CanActivate,CanActivateChild{
  	...
  	canActivate(route: ActivatedRouteSnapshot,state:RouterStateSnapshot):Observable<boolean> | Promise<boolean> | boolean
	{
		...
		//user must be authenticated(logged in) and be a customer
			...
			return new Promise(function(resolve,reject)
			{
				...
				that.appOp.checkUserAccess()
					.subscribe((response:any)=> {
						...
						if(that.authService.isAuthenticated() && that.authService.isCustomer()){
							resolve(true);
						}else {
